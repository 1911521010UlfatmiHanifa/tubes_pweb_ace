<div>
    <div class="max-w-6xl mx-auto py-10 sm:px-6 lg:px-8">
        ...
        @if($isDel)
            @include('livewire.kelas.confirmation_delete')
        @endif
                        ...
                        <table class="min-w-full divide-y divide-gray-200 w-full">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                    {{ $kelas->tahun }} - {{$kelas->semester}}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $kelas->sks }}
                                </td>
                                    <td>
                                        <button onclick="location.href=' {{ route( 'dkelas', [$kelas->id]) }} '" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                                        Detail
                                        </button>
                                        <button wire:click="edit({{ $kelas->id }})" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                                        Edit
                                        </button>
                                        <button wire:click="showDel({{ $kelas->id }})"  class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                                        Hapus
                                        </button>
                                    </td>
                            </tr>
