<div class="fixed z-10 inset-0 overflow-y-auto">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full rounded" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
        <form>
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div>
              ...
              <div>
                  <div class="mb-2">
                      <input wire:model="mahasiswaId" type="hidden" class="shadow appearance-none border w-full py-2 px-3 text-blue-900">
                  </div>
                  <div class="mb-2">
                      <label for="nama" class="block py-1">Nama Mahasiswa</label>
                      <input wire:model="nama" name="nama" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded" placeholder="Input Nama Mahasiswa">
                      @error('nama') <h1 class="text-red-500">{{$message}}</h1>@enderror
                  </div>
                  <div class="mb-2">
                    <label for="nim" class="block py-1">NIM</label>
                    <input wire:model="nim" name="nim" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded" placeholder="Input NIM">
                    @error('nim') <h1 class="text-red-500">{{$message}}</h1>@enderror
                </div>
                <div class="mb-2">
                    <label for="email" class="block py-1">Email</label>
                    <input wire:model="email" name="email" min="0" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded" placeholder="Input Email">
