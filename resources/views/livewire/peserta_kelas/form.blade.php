<div class="fixed z-10 inset-0 overflow-y-auto">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-gray-100 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full rounded" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
        <form>
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div>
              ...
              <div>
                  <div class="mb-2">
                      <input wire:model="krs_id" type="hidden" class="shadow appearance-none border w-full py-2 px-3 text-blue-900">
                  </div>
                  <div class="mb-2">
                      <label for="kelasId" class="block py-1">Kode Kelas</label>
                      <input wire:model="kelasId" name="kelasId" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded" placeholder="{!! $dkelas->kode_kelas !!}" readonly>
                      @error('kode_kelas') <h1 class="text-red-500">{{$message}}</h1>@enderror
                  </div>
                  <div class="mb-2">
                  <label for="mahasiswa_id" class="block py-1">Nama Mahasiswa</label>
                    <select wire:model="mahasiswa_id" name="mahasiswa_id" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded">
                      <option selected>Pilih Nama Mahasiswa</option>
                      @foreach($dafmahasiswa as $m)
                          <option value="{!! $m->id !!}">{!! $m->nama !!}</option>
                      @endforeach
                    </select>
                    @error('mahasiswa_id') <h1 class="text-red-500">{{$message}}</h1>@enderror
                </div>
              </div>
        </div>
