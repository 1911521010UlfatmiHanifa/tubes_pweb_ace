    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div>
              <h1 class="font-bold text-center mb-4">Tambah Pertemuan</h1>
            </div>
              <div>
                  <div class="mb-2">
                      <input wire:model="pertemuanId" type="hidden" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded">
                  </div>
                  <div class="mb-2">
                      <input wire:model="kelas_id" type="hidden" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded">
                  </div>
                  <div class="mb-2">
                      <label for="pertemuan_ke" class="block py-1">Pertemuan</label>
                      <input wire:model="pertemuan_ke" type="number" min="1" name="pertemuan_ke" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded" placeholder="Input Pertemuan Ke">
                      @error('pertemuan_ke') <h1 class="text-red-500">{{$message}}</h1>@enderror
                  </div>
                  <div class="mb-2">
                    <label for="tanggal" class="block py-1">Tanggal</label>
                    <input type="date" wire:model="tanggal" name="tanggal" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded" placeholder="Input Tanggal">
                    @error('tanggal') <h1 class="text-red-500">{{$message}}</h1>@enderror
                </div>
                <div class="mb-2">
                    <label for="materi" class="block py-1">Materi</label>
                    <input wire:model="materi" name="materi" class="bg-green-100 shadow appearance-none border w-full py-2 px-4 text-black text-sm rounded" placeholder="Input Materi">
